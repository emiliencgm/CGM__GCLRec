    ...
    try:
        ...
        for epoch in range(world.config['epochs']):
            ...
            #plot MLP(pop)
                with torch.no_grad():
                    if epoch % 5 == 1:
                        max_pop_i = math.log(precal.popularity.max_pop_i)
                        ...
                        Img = wandb.Image(plt1, caption="epoch:{}".format(epoch))  # attention!!!
                        wandb.log({"MLP(pop_i)": Img})
                        ...
                        Img = wandb.Image(plt2, caption="epoch:{}".format(epoch))  # attention!!!
                        wandb.log({"MLP(centroid)": Img})

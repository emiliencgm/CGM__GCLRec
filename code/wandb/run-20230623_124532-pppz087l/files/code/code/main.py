...
def main():
    ...
    try:
        ...
        for epoch in range(world.config['epochs']):
            ...
            # #plot MLP(pop)
            #     if epoch % 5 == 1:
            #         hotest_user = precal.popularity.UserPopGroupDict[9][-1]
            #         hotest_user = int(hotest_user)
            #         for user_pop in range(1, precal.popularity.user_pop_degree_label[hotest_user], 10):
            #             # print(user_pop)
            #             input_pop = torch.Tensor([[-math.log(user_pop), 0., 0., 0., 0.]]).to(world.device)
            #             output_pop = total_loss.MLP_model(input_pop)[0].item()
            #             # print(output_pop)
            #             wandb.log( { f"{world.config['dataset']}"+f"/MLP(user_pop)_@epoch {epoch}": output_pop, "log(pop_u)":math.log(user_pop)} )
            #         hotest_item = precal.popularity.ItemPopGroupDict[9][-1]
            #         hotest_item = int(hotest_item)
            #         for item_pop in range(1, precal.popularity.item_pop_degree_label[hotest_item], 10):
            #             input_pop = torch.Tensor([[0., -math.log(item_pop), 0., 0., 0.]]).to(world.device)
            #             output_pop = total_loss.MLP_model(input_pop)[0].item()
            #             # print(item_pop, output_pop)
            #             wandb.log( { f"{world.config['dataset']}"+f"/MLP(item_pop)_@epoch {epoch}": output_pop, "log(pop_i)":math.log(item_pop)} )
            #         for centroid in np.linspace(0,1,100):
            #             input_centroid = torch.Tensor([[0., 0., centroid, 0., 0.]]).to(world.device)
            #             output_centroid = total_loss.MLP_model(input_centroid)[0].item()
            #             # print(centroid, output_centroid)
            #             wandb.log( { f"{world.config['dataset']}"+f"/MLP(centroid)_@epoch {epoch}": output_centroid, "centroid":centroid} )

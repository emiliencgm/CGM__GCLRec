...
if args.task == 'yelp2018':
    os.system(f'python main.py --notes L={str(args.L)}_k={str(args.k)} --alpha 0.5 --temp_tau 0.1 --lambda1 0.1 --epsilon_GCLRec {args.eps} --w_GCLRec {args.w} --k_aug {args.k} --project {project} --name Adaptive_Aug+BPR_Contrast --model GCLRec --loss BPR_Contrast --augment Adaptive --dataset {args.task} --init_method Normal --lr 0.001 --weight_decay 1e-4 --cuda {args.device} --num_layers {args.LGN_layer} --latent_dim_rec 64 --batch_size 2048 --comment _ --if_valid {args.valid} --tag Adaptive_Aug --group Ours --job_type {args.task} --if_visual {args.visual} --visual_epoch 1')

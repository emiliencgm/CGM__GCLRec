...
if args.task == 'yelp2018':
    os.system(f'python main.py --notes SimGCL+All_Weighted_InfoNCE_+_AdaLoss_mlp --adaptive_method mlp --centroid_mode {args.centroid} --commonNeighbor_mode {args.CN} --temp_tau {args.tau} --project {project} --name SimGCL+All_Weighted_Loss --model SimGCL --loss AllWeight --dataset {args.task} --init_method Normal --lr 0.001 --weight_decay 1e-4 --cuda {args.device} --num_layers 3 --latent_dim_rec 64 --batch_size 2048 --comment _ --if_valid {args.valid} --tag Adaptive_loss --group Ours --job_type {args.task} --if_visual {args.visual} --visual_epoch 3')

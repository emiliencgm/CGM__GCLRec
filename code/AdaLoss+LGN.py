...
if args.task == 'yelp2018':
    os.system(f'python main.py --notes LightGCN+Adaptive_loss__mlp__log_pop+centroid_eig+CN_SC__ --adaptive_method mlp --centroid_mode {args.centroid} --commonNeighbor_mode {args.CN} --temp_tau {args.tau} --project GCLRec_No_Valid --name LightGCN+Adaptive_loss_mlp --model LightGCN --loss Adaptive --dataset {args.task} --init_method Normal --lr 0.001 --weight_decay 1e-4 --cuda {args.device} --num_layers 3 --latent_dim_rec 64 --batch_size 2048 --comment _ --if_valid 0 --tag Adaptive_loss --group Ours --job_type {args.task} --if_visual {args.visual} --visual_epoch 3')
